...
public class EditorActivity extends AppCompatActivity {
    ...
    public KitInventoryDBHelper dbHelper;
    ...
    private void insertKit() {
        ...
        values.put(KitInventoryEntry.COLUMN_PRODUCT_NAME, productName);
        values.put(KitInventoryEntry.COLUMN_PRODUCT_PRICE, productPriceInt);
        values.put(KitInventoryEntry.COLUMN_PRODUCT_QUANTITY, productQuantityInt);
        values.put(KitInventoryEntry.COLUMN_SUPPLIER_NAME, supplierName);
        values.put(KitInventoryEntry.COLUMN_SUPPLIER_PHONE_NUMBER, supplierContact);

        long newRowId = db.insert(KitInventoryEntry.TABLE_NAME, null, values);
        if (newRowId == -1) {
            Toast.makeText(this, getString(R.string.error_saving_kit), Toast.LENGTH_SHORT).show();
        } else {
            Toast.makeText(this, getString(R.string.kit_saved) + " " + newRowId, Toast.LENGTH_SHORT).show();
            finish();
        }
    }
